...
[![CI](https://github.com/phumsoft/phumpie/actions/workflows/CI.yml/badge.svg)](https://github.com/phumsoft/phumpie/actions/workflows/CI.yml)
...
`Repository Pattern` are classes or components that encapsulate the logic required to access data sources. They centralize common data access functionality, providing better maintainability and decoupling the infrastructure or technology used to access databases from the domain model layer.

So, here is a custom package for custom code structure, reduce rewrite common logic for repository application.
...
- <a href="#methods">2. Methods</a>
  - <a href="#phumpiepositorycontractsrepositoryinterface">RepositoryInterface</a>
  - <a href="#phumpiepositorycontractsrepositorycriteriainterface">RepositoryCriteriaInterface</a>
  - <a href="#phumpiepositorycontractscacheableinterface">CacheableInterface</a>
  - <a href="#phumpiepositorycontractspresenterinterface">PresenterInterface</a>
  - <a href="#phumpiepositorycontractscriteriainterface">CriteriaInterface</a>
...
- <a href="#cache">4. Cache</a>
  - <a href="#cache-usage">Usage</a>
  - <a href="#cache-config">Config</a>
- <a href="#validators">5. Validators</a>
...
- <a href="#presenters">6. Presenters</a>
...
You must first configure the storage location of the repository files. By default is the "app" folder and the namespace "App". Please note that, values in the `paths` array are actually used as both _namespace_ and file paths. Relax though, both forward and backward slashes are taken care of during generation.
...
In order for it to query using the **AND**, pass the _searchJoin_ parameter as shown below:

- `http://phumpie.local/users?search=age:17;email:john@gmail.com&searchJoin=and`

Search record with `has` or `doesntHave`
...
You can change the cache settings in the file _config/repository.php_ and also directly on your repository.

_config/repository.php_
...
Validation is now ready. In case of a failure an exception will be given of the type: _Prettus\Validator\Exceptions\ValidatorException_
...
Use _skipPresenter_ before any other chaining method
